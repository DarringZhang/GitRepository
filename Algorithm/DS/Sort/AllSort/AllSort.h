...
//第i次循环, 从A[i, … , n - 1]中找出最?的, 将它和A[i]互换.i= 0, …, n - 2
...
void Insert_Sort(vector<int> &A,int begin,int end){
    for(int j = 1; j < LEN; ++j){
        ...
        while(i >= 0 && A [i] > key){//key 太小了，key要往前移动了
            A[i + 1] = A[i];//key 已经被记录下来了，第一次循环前一个数覆盖key 即A[i+1]的位置。这样就空出一个位置。再符合条件一步一步往后
            i--;
        }
...
void Shell_Sort(vector<int> &A,int begin,int end){
    ...
    while(h >= 1){
        for(int i = h; i < LEN; ++i){//i都跑完后，代表每个列变得有序
            for(int j = i; j >= h && A[j] < A[j-h]; j-=h){//每列数各自间进行比较，每次i都将队列中当前触碰到的最后一个数调到合适位置
                swap(A[j],A[j-h]);
            }
        }
        h /= 3;//缩短列的个数
...
void Merge(vector<int> &A,int begin,int medium, int end){
    int n1 = medium - begin +1;//左边数组的长度，0 ~ medium,要取得到medium, 实际长度medium+1
...
//分治思想：对于n个元素的数组A, 每次把数组分成两部分: ?部分?于等于A[n-1], ?部分?于A[n-1], 那么A[n-1]
//就可以放在这两个数组的中间. 然后递归地快速排序这两个?规模的数组.
