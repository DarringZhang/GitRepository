...
*/
    ...
    Node *find_first_node_postOrder(Node *p){//找到最左边且最底部的点
        while(p) {
            if(p->l_child){
                p = p->l_child;
            }
...
bool operator < (const data& first, const data& other){//q???&???????????????α???????
    return first.key < other.key;//С???????????key?
}
...
void testSearchTime(int size){
   ...
   ab  = geStr(size);
    ...
    start = clock();
       ...
       aBin.InOrder_Stack(aBin.GetRoot());

    printf("Search Time(Binary): %ld miu seconds\n", clock() -  start);

    start = clock();
  bTri.inOrder_No_Stack();
        //bAVL.BST_Search_NonRecur(bTri.GetRoot(),SearchArray[i]);

    printf("Search Time(Tri): %ld miu seconds\n", clock() -  start);
